...
> Dernière mise à jour : 2026-05-29 · Dernier commit : `1de8326` · Branches `feat/modular-foundation` **et** `main` synchronisées sur origin.
...
## ▶️ PROCHAINES TÂCHES (dans l'ordre)
1. **Module Transaction** (dépenses/achats) : table + service + onglet → écriture achat→GL (DR charges + CTI/RTI 1200/1210, CR banque 1000 / fournisseurs 2000) + pièces jointes (Supabase Storage).
2. **Exports** PDF/CSV des états financiers et du grand livre.
3. **Phase 5 — rapports fiscaux** :
   - Avantage automobile : export **TP-41.C** (employeur) + report sur **RL-1 case W / T4 code 34** (l'employé reçoit via son feuillet, PAS le TP-41).
   - Agrégats **TPS/TVQ** pour les déclarations (taxe perçue vs CTI/RTI).
   - Base **T4 / RL-1** (XML obligatoire si > 5 feuillets, 2 schémas distincts ARC + RQ).
   - **Retenues à la source** : ventiler `CR 2300 net` → net + 2200 (fédéral RPC/AE/impôt) + 2210 (QC RRQ/RQAP/impôt/FSS).
4. **Affiner** : PST/RST provincial dans un compte de taxe distinct (regroupé pour l'instant avec TPS/TVH dans 2100).
