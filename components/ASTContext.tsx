...
import { ASTFormData, ASTFormSection } from '@/app/types/astForm';
...
// =================== ACTIONS TYPES ===================
type ASTAction =
  | { type: 'UPDATE_STEP_DATA'; section: ASTFormSection; data: ASTFormData[ASTFormSection] }
...
interface ASTContextValue {
  ...
  // Actions principales
  updateStepData: <K extends ASTFormSection>(section: K, data: ASTFormData[K]) => void;
...
export function ASTProvider({
  ...
  const updateStepData = useCallback(<K extends ASTFormSection>(section: K, data: ASTFormData[K]) => {
...
async function saveToTenantDatabase(
  ...
  section: ASTFormSection | 'complete_ast',
  data: unknown,
  astNumber: string
) {
  try {
