...
- [~] **Endroit des travaux + Google Maps** : champ « Endroit des travaux » + lien **« Voir sur Google Maps »** (zéro clé API) FAIT. Colonnes `lieuLat/lieuLng` prêtes (migration 089). _Reste (besoin clé `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY`) : géocodage/autocomplete + carte intégrée ; **préremplissage depuis Projets** (adresse/client) si module Projets activé._
