    ...
    <div className="p-4 sm:p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          ...
          {/* Assistant Prix IA (recherche web du prix coûtant) : sélection / toute la liste / périmés */}
          {onPriceAssistant && (() => {
            // Article "périmé" = prix jamais mis à jour par l'IA, ou il y a plus de 90 jours (3 mois).
            const STALE_MS = 90 * 86400000;
            const isStale = (it) => { const t = it.lastPriceUpdate ? new Date(it.lastPriceUpdate).getTime() : 0; return !t || (Date.now() - t) > STALE_MS; };
            const staleItems = filteredItems.filter(isStale);
            return (
              <>
                <button
                  onClick={() => onPriceAssistant(selectedItems.length ? filteredItems.filter(it => selectedItems.includes(it.id)) : filteredItems)}
                  disabled={filteredItems.length === 0}
                  title={language === 'fr' ? (selectedItems.length ? 'Prix de la sélection (IA web)' : 'TOUTE la liste affichée (IA web)') : 'AI web price update'}
                ...
                {!selectedItems.length && (
                  <button
                    onClick={() => onPriceAssistant(staleItems)}
                    disabled={staleItems.length === 0}
                    title={language === 'fr' ? 'Mettre à jour SEULEMENT les articles dont le prix date de plus de 3 mois (économise les frais IA)' : 'Update only articles whose price is older than 3 months'}
                    className="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-lg bg-purple-100 text-purple-700 hover:bg-purple-200 dark:bg-purple-900/30 dark:text-purple-300 disabled:opacity-50 transition-all whitespace-nowrap"
                  >
                    <Zap size={15} />
                    <span className="hidden sm:inline">{language === 'fr' ? `Périmés >3 mois (${staleItems.length})` : `Stale >3mo (${staleItems.length})`}</span>
                  </button>
                )}
              </>
            );
