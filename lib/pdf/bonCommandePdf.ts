...
function drawHeader(doc: Doc, logo: string | null, title: string, subtitle: string, accent?: [number, number, number], ruleWidth?: number) {
  ...
  // Filet d'accent du module (Modèles PDF) — couleur + épaisseur réglables, défaut bleu.
  doc.setDrawColor(...(accent || COL.primary)); doc.setLineWidth((ruleWidth ?? 1) * 0.35); doc.line(0, 22, PAGE_W, 22);
}
...
export async function exportBonCommandePdf(opts: {
  ...
  const st = await import('@/lib/pdfStyle').then(m => m.pdfStyleFor(opts.tenant, 'bon_commande')).catch(() => undefined);

  drawHeader(doc, logo, tr('Bon de commande', 'Purchase order'), `${b.numero} · ${opts.tenant} · ${new Date().toLocaleDateString('fr-CA')}`, st?.accent, st?.ruleWidth);
