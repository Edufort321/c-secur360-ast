...
import { resolveAccess, canShareholders, effectiveTenant, effectiveLevelFor, type Access } from '@/lib/hrAccess';
...
async function guard(req: NextRequest, reqTenant?: string | null) {
  ...
  if (!canShareholders(level)) return { err: NextResponse.json({ error: 'Accès refusé (dividendes)' }, { status: 403 }) };
  return { acc, tenant };
}
