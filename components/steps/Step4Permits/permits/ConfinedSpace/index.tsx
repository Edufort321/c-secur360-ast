...
export default function ConfinedSpacePermit() {
  ...
  const handleSavePermit = async () => {
    ...
    try {
      const permitId = permitData.id || `permit_${Date.now()}`;
      const permitLink = `https://c-secur360-ast.vercel.app/permits/${permitId}`;
      const qrCode = await generateQRCode(permitLink);

      const permitToSave: PermitData = {
        ...permitData,
        id: permitId,
        qr_code: qrCode,
      ...
      const { error } = await supabase
        .from("permits")
        .upsert([permitToSave], { onConflict: "id" });

      if (error) {
        setSaveMessage("❌ Erreur lors de la sauvegarde : " + error.message);
      } else {
        setSaveMessage("✅ Permis sauvegardé avec succès !");
        setPermitData(permitToSave);
      }
    } catch (err: any) {
      setSaveMessage("❌ Erreur inattendue : " + (err.message || err.toString()));
    } finally {
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto", padding: 24 }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <label>
          ...
          <input
            type="text"
            name="title"
            value={permitData.title || ""}
            onChange={handleFieldChange}
            style={{ width: "100%", padding: 8, marginTop: 4 }}
            placeholder="Titre du permis"
          />
        </label>
      </div>
      <div style={{ marginBottom: 16 }}>
        <label>
          ...
          <textarea
            ...
            placeholder="Description du permis"
          />
        </label>
      </div>

      {/* Ajoute ici d'autres champs spécifiques au permis si besoin */}
      ...
      {saveMessage && (
        <div style={{ marginTop: 12, color: saveMessage.startsWith("✅") ? "limegreen" : "red" }}>
          {saveMessage}
        </div>
      )}
      ...
      {permitData.qr_code && (
        <div style={{ marginTop: 16 }}>
          <p>QR code du permis :</p>
          <img src={permitData.qr_code} alt="QR code du permis" style={{ width: 128, height: 128 }} />
          <div style={{ wordBreak: "break-all", fontSize: 12, marginTop: 8 }}>
            Lien : <a href={`https://c-secur360-ast.vercel.app/permits/${permitData.id}`} target="_blank" rel="noopener noreferrer">
              https://c-secur360-ast.vercel.app/permits/{permitData.id}
            </a>
          </div>
        </div>
      )}
