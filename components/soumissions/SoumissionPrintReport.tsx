...
// en-tête/pied répétés (run-table), barres de section, tableaux stylés, sauts de page propres.
...
export function SoumissionPrintReport(props: {
  ...
  const { soumission: s, items, cat, companyName, logo, headerColor, sections, breakdownMode, conditions, cover } = props;
  ...
  const SP: Record<string, React.CSSProperties> = {
    ...
    itemTotal: { fontWeight: 700, fontSize: 12 },
    ...
    condBlock: { breakInside: 'avoid', marginBottom: 8 },
    condTitle: { fontWeight: 700, fontSize: 10.5, color: band, marginBottom: 2 },
    condBody: { fontSize: 10, lineHeight: 1.35, whiteSpace: 'pre-wrap', color: '#333' },
  };
  ...
  const meta = [s.client_snapshot?.lieu, s.year ? `Année ${s.year}` : '', companyName].filter(Boolean).join('  ·  ');

  return (
    <div className="soum-print-only" style={SP.wrap}>
      {/* ── Page de présentation (lettre) — optionnelle ── */}
      {sections.cover && cover && (
        <div className="soum-cover" style={{ pageBreakAfter: 'always' }}>
          <div style={{ height: 6, background: band, marginBottom: 18 }} />
          {logo && <img src={logo} alt="" style={{ height: 40, marginBottom: 16 }} />}
          <div style={{ textAlign: 'right', fontSize: 10, color: '#555' }}>{cover.ville ? `${cover.ville}, ` : ''}{cover.date || new Date().toLocaleDateString('fr-CA')}</div>
          <div style={{ marginTop: 20, fontWeight: 700 }}>{cover.to || s.client_snapshot?.name || ''}</div>
          <div style={{ marginTop: 6, fontSize: 10, color: '#555' }}>Objet : Soumission {s.numero}</div>
          <div style={{ marginTop: 16, fontSize: 11, lineHeight: 1.5, whiteSpace: 'pre-wrap' }}>{cover.body || ''}</div>
          <div style={{ marginTop: 16, fontSize: 11 }}>{cover.salutation || ''}</div>
          <div style={{ marginTop: 28 }}>
            {cover.signature_url && <img src={cover.signature_url} alt="" style={{ height: 46, display: 'block' }} />}
            <div style={{ fontWeight: 700 }}>{cover.signataire_nom || ''}</div>
            <div style={{ fontSize: 10, color: '#555' }}>{cover.signataire_titre || ''}{companyName ? ` · ${companyName}` : ''}</div>
          </div>
        </div>
      )}

      {/* ── Corps : table porteuse pour répéter en-tête/pied à chaque page ── */}
      <table className="soum-runtable" style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tfoot>
          <tr><td>
            <div className="run-foot">{companyName || 'C-Secur360'} · Soumission {s.numero} · {new Date().toLocaleDateString('fr-CA')}</div>
          </td></tr>
        </tfoot>
        <tbody>
          <tr><td>
            ...
            <div style={{ fontWeight: 700, fontSize: 12 }}>{s.client_snapshot?.name || '—'}</div>
            {meta && <div style={{ fontSize: 10, color: '#6b7280', marginBottom: 6 }}>{meta}</div>}
                  ...
                  return (
                    <div key={i} className="soum-avoid" style={{ breakInside: 'avoid' }}>
                      <div style={SP.itemHead}>
                        <span style={SP.itemName}>{it.name || `Item ${i + 1}`}</span>
                        {mode !== 'global_desc' && <span style={SP.itemTotal}>{money(itTotal)}</span>}
                      </div>
                      {mode === 'par_item' ? (
                        <div>
            ...
            {sections.conditions && conditions.length > 0 && (
              <div style={{ marginTop: 8 }}>
                ...
                {conditions.map((c, i) => (
                  <div key={i} style={SP.condBlock}>
                    {c.titre && <div style={SP.condTitle}>{c.titre}</div>}
                    <div style={SP.condBody}>{c.contenu}</div>
                  </div>
                ))}
              </div>
            )}
...
// CSS d'impression (à injecter dans un <style>). Le rapport est porté en enfant direct de <body>.
...
@media print {
  ...
  .soum-cover { min-height: 245mm; box-sizing: border-box; }
