...
export default function AddLocationModal({ isOpen, initial, onCancel, onSave }: Props) {
  ...
  if (!isOpen || !mounted) {
    return null; // Ne pas tenter de mettre le focus si le modal n'est pas ouvert
  }
  ...
  const handleSave = (): void => {
    ...
    addressInputRef.current?.focus(); // Assurer le focus sur le champ d'adresse après la sauvegarde
  };
...
Je vais maintenant écrire ce contenu dans le fichier `AddLocationModal.tsx`. 

<write_to_file>
<path>components/modals/AddLocationModal.tsx</path>
<content>
'use client';
