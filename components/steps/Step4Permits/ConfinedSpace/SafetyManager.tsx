...
try {
  if (!supabaseKey) {
    logger.warn('SafetyManager: NEXT_PUBLIC_SUPABASE_ANON_KEY manquant. Configurez-le dans votre fichier .env.local. Utilisation du localStorage.');
  } else {
    const url = new URL(supabaseUrlEnv ?? FALLBACK_URL).toString();
    if (!supabaseUrlEnv) {
      logger.warn(`SafetyManager: NEXT_PUBLIC_SUPABASE_URL manquant. Configurez-le dans votre fichier .env.local. Utilisation de ${FALLBACK_URL}`);
    }
    supabase = createClient(url, supabaseKey);
    supabaseEnabled = true;
    logger.info('✅ SafetyManager: Supabase configuré');
  }
} catch (error) {
  logger.error('⚠️ SafetyManager: Supabase non configuré, utilisation du localStorage', error);
  ...
  persist(
    (set, get) => ({
      ...
      updateSiteInformation: (data) => {
        logger.info('🔄 SafetyManager: updateSiteInformation appelé', data);
        ...
        if (state.autoSaveEnabled) {
          ...
          const timer = setTimeout(() => {
            logger.info('💾 Auto-save silencieux déclenché après débounce');
        ...
        logger.info('✅ SafetyManager: siteInformation mis à jour silencieusement');
      },

      updateAtmosphericTesting: (data) => {
        logger.info('🔄 SafetyManager: updateAtmosphericTesting appelé', data);
        ...
        logger.info('✅ SafetyManager: atmosphericTesting mis à jour silencieusement');
      },

      updateEntryRegistry: (data) => {
        logger.info('🔄 SafetyManager: updateEntryRegistry appelé', data);
        ...
        logger.info('✅ SafetyManager: entryRegistry mis à jour silencieusement');
      },

      updateRescuePlan: (data) => {
        logger.info('🔄 SafetyManager: updateRescuePlan appelé', data);
        ...
        logger.info('✅ SafetyManager: rescuePlan mis à jour silencieusement');
      },

      // =================== MÉTHODES POUR ENTRYREGISTRY - OPTIMISÉES ===================
      updateRegistryData: (data) => {
        logger.info('🔄 SafetyManager: updateRegistryData appelé', data);
        get().updateEntryRegistry(data);
      },

      updatePersonnel: (person) => {
        logger.info('🔄 SafetyManager: updatePersonnel appelé', person);
      ...
      updateEquipment: (equipment) => {
        logger.info('🔄 SafetyManager: updateEquipment appelé', equipment);
        get().updateEntryRegistry({ equipment });
      },

      updateCompliance: (key: string, value: boolean) => {
        logger.info('🔄 SafetyManager: updateCompliance appelé', key, value);
      ...
      recordEntryExit: (personId, action) => {
        logger.info('🔄 SafetyManager: recordEntryExit appelé', personId, action);
      ...
      updateSiteInfo: (data: any) => {
        logger.info('🔄 SafetyManager: updateSiteInfo (alias) appelé', data);
        get().updateSiteInformation(data);
      },

      updateAtmosphericData: (data: any) => {
        logger.info('🔄 SafetyManager: updateAtmosphericData (alias) appelé', data);
        get().updateAtmosphericTesting(data);
      },

      updateRegistryInfo: (data: any) => {
        logger.info('🔄 SafetyManager: updateRegistryInfo (alias) appelé', data);
        get().updateEntryRegistry(data);
      },

      updateRescueData: (data: any) => {
        logger.info('🔄 SafetyManager: updateRescueData (alias) appelé', data);
      ...
      flushPendingUpdates: () => {
        ...
        if (Object.keys(state.pendingUpdates).length > 0) {
          logger.info('🔄 Flush des updates en attente:', state.pendingUpdates);
      ...
      silentSave: async () => {
        ...
        if (state.isSaving) {
          logger.info('🚫 Sauvegarde silencieuse déjà en cours');
          return;
        }

        logger.info('💾 Sauvegarde silencieuse en cours...');
        ...
        try {
          ...
          if (supabaseEnabled && supabase) {
            try {
              ...
              logger.info('✅ Permit sauvegardé silencieusement dans Supabase');
            } catch (supabaseError) {
              logger.error('❌ Erreur Supabase, fallback localStorage:', supabaseError);
            ...
            logger.info('✅ Permit sauvegardé silencieusement dans localStorage');
          }
          ...
          logger.error('❌ Erreur sauvegarde silencieuse:', error);
        }
      },
      ...
      saveToDatabase: async () => {
        ...
        if (state.isSaving) {
          logger.info('🚫 Sauvegarde déjà en cours, ignorée');
          return null;
        }
        ...
        try {
          ...
          logger.info('💾 Sauvegarde manuelle en cours...', permit.permit_number || 'nouveau');
          ...
          if (supabaseEnabled && supabase) {
            try {
              ...
              logger.info('✅ Permit sauvegardé dans Supabase');
            } catch (supabaseError) {
              logger.error('❌ Erreur Supabase, fallback vers localStorage:', supabaseError);
              localStorage.setItem(`permit_${permit.permit_number}`, JSON.stringify(permit));
            }
          } else {
            // Fallback localStorage
            localStorage.setItem(`permit_${permit.permit_number}`, JSON.stringify(permit));
            localStorage.setItem('currentPermit', JSON.stringify(permit));
            logger.info('✅ Permit sauvegardé dans localStorage');
          }
      ...
      loadFromDatabase: async (permitNumber: string) => {
        ...
        try {
          logger.info('📥 Chargement du permis:', permitNumber);
          ...
          if (supabaseEnabled && supabase) {
            try {
              ...
              logger.error('❌ Erreur chargement Supabase:', supabaseError);
            }
          }
        ...
        try {
          ...
          if (supabaseEnabled && supabase) {
            try {
              ...
              logger.error('❌ Erreur historique Supabase:', supabaseError);
            }
          }
              ...
              for (let i = 0; i < localStorage.length; i++) {
                ...
                if (key && key.startsWith('permit_') && key !== `permit_${currentPermit.permit_number}`) {
                  try {
                    ...
                    logger.warn('⚠️ Erreur parsing permit:', key);
                  }
                }
              }
            }
