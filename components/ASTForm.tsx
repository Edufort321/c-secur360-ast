...
const ASTForm: React.FC<ASTFormProps> = ({ 
  ...
  // Langue COMMUNE : pilotée par le header global (LanguageContext)
  const { lang: currentLanguage, setLang: setCurrentLanguage } = useLanguage();
  ...
  // =================== LANGUE GÉRÉE GLOBALEMENT (LanguageContext) ===================
  // (plus de resynchronisation locale : la langue vient du header commun)
  ...
  return (
    <div style={{
      ...
      {/* Ancien header retiré (logo/titre/sélecteur de langue gérés par le header global).
          On garde uniquement le numéro d'AST auto-généré. */}
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Shield size={16} color="#3b82f6" />
          <span style={{ fontSize: '12px', color: '#94a3b8' }}>{t.astNumber}</span>
          <span style={{ fontSize: '15px', fontWeight: 700 }}>{astData.astNumber}</span>
        </div>
