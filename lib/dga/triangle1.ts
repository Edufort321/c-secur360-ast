// Triangle de Duval 1 — logique PURE (géométrie + zones + classification), réutilisée par le composant
// de rendu et par les autres méthodes d'interprétation (consensus / auto-sélection). Sans React.
// Convention d'axes (IEC 60599 / IEEE C57.104-2019) : CH4 haut · C2H2 bas-gauche · C2H4 bas-droite.
// ⚠️ Frontières = définition publique de la méthode Duval (en % de gaz) ; à confirmer contre la norme officielle.

export const TOP = { x: 200, y: 30 };    // CH4
export const LEFT = { x: 40, y: 310 };   // C2H2
export const RIGHT = { x: 360, y: 310 }; // C2H4

export type Gases = { CH4: number; C2H4: number; C2H2: number };
export type PctVertex = { ch4: number; c2h2: number; c2h4: number };

export function toPercent({ CH4, C2H4, C2H2 }: Gases) {
  const total = CH4 + C2H4 + C2H2;
  if (total <= 0) return null;
  return { pCH4: (CH4 / total) * 100, pC2H2: (C2H2 / total) * 100, pC2H4: (C2H4 / total) * 100 };
}
export function pointXY(pCH4: number, pC2H2: number, pC2H4: number) {
  const a = pCH4 / 100, b = pC2H2 / 100, c = pC2H4 / 100;
  return { x: a * TOP.x + b * LEFT.x + c * RIGHT.x, y: a * TOP.y + b * LEFT.y + c * RIGHT.y };
}

export const ZONES: { fault: string; fill: string; stroke: string; text: string; label: string; verts: PctVertex[] }[] = [
...
export function zonePoints(verts: PctVertex[]) {
