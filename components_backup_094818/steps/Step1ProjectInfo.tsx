...
function Step1ProjectInfo({ formData, onDataChange, language, tenant, errors = {}, userId, userRole }: Step1ProjectInfoProps) {
  ...
  const notifyParentStable = useCallback((updatedData: any, source = 'unknown') => {
    ...
    if (lastUpdateRef.current === updateKey) {
      console.log(`🛡️ Step1 - Update #${updateId} doublon évité`);
      return;
    }
    ...
    const newTimer = setTimeout(() => {
      try {
        console.log(`✅ Step1 - Update #${updateId} envoyé au parent`);
  ...
  const openModal = useCallback(() => {
    ...
    requestAnimationFrame(() => {
      ...
      setTimeout(() => {
        ...
        console.log('✅ Step1 - Modal forcée z-index maximum');
      }, 100);
    });
  }, []);
  ...
  const addWorkLocation = useCallback(() => {
    if (!modalState.data.name.trim() || !modalState.data.zone.trim()) {
      console.log('❌ Step1 - Données modal insuffisantes');
      return;
    }

    if (modalState.isSaving) {
      console.log('⏳ Step1 - Modal déjà en cours de sauvegarde');
      return;
    }
  ...
  useEffect(() => {
    ...
    const handleFileChange = (e: Event) => {
      ...
      console.log(`🔥 Step1 - Photos sélectionnées: ${files.length}, contexte:`, captureContext);
  ...
  useEffect(() => {
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      if (target.closest('.modal-overlay-ultra-critical') || target.closest('.btn-primary')) {
        console.log('🔍 Step1 - Click détecté:', {
  ...
  const LockoutStatusToggle = React.memo(({ point }: { point: LockoutPoint }) => (
    <div className="form-field">
      <label className="field-label">
  ...
  const LockoutProgressIndicator = React.memo(({ point }: { point: LockoutPoint }) => {
    const progress = getProcedureProgress(point);
