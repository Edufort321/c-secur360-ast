...
const ConfinedSpace: React.FC<ConfinedSpaceProps> = ({
  ...
  logger.info('🔄 SafetyManager réel activé - Mode validation seulement (pas de sync agressive)');
  ...
  useEffect(() => {
    ...
    if (isSafetyManagerEnabled && safetyManager && permitData.permit_number) {
      try {
        logger.info('🔄 SafetyManager: Validation en cours...');
        ...
        logger.info('✅ SafetyManager: Validation terminée', validation);
        ...
        logger.info('❌ Erreur SafetyManager validation:', error);
      }
    }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (!isActuallyReadOnly && !showManager && permitData.permit_number) {
        logger.info('🔄 Auto-save sécurisé déclenché');
  ...
  const updatePermitData = useCallback((updates: Partial<PermitData>) => {
    logger.info('📝 updatePermitData appelé avec:', updates);
    ...
    logger.info('🔄 SafetyManager réel actif - mise à jour locale et validation');
  ...
  const handleSectionDataChange = useCallback((field: string, value: any) => {
    logger.info(`📊 handleSectionDataChange: ${field} =`, value);
  ...
  const handleUpdatePermitData = useCallback((data: any) => {
    logger.info(`📋 handleUpdatePermitData:`, data);
  ...
  const savePermitData = async (showNotification = true, isAutoSave = false) => {
    if (isActuallyReadOnly) {
      logger.info('🚫 Sauvegarde bloquée: mode lecture seule');
      return;
    }
    
    logger.info(`💾 savePermitData: showNotification=${showNotification}, isAutoSave=${isAutoSave}`);
    ...
    try {
      ...
      logger.info('🔇 SafetyManager désactivé - sauvegarde locale seulement');
  ...
  const navigateToSection = (section: 'site' | 'rescue' | 'atmospheric' | 'registry' | 'finalization') => {
    logger.info(`🧭 Navigation vers: ${section}`);
