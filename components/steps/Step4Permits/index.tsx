...
const Step4Permits: React.FC<Step4PermitsProps> = ({
  ...
  const handlePermitSelect = async (permitId: string) => {
    ...
    if (permitId === 'confined-space') {
      try {
        logger.info('🔄 Tentative de chargement ConfinedSpace...');
        ...
        logger.info('✅ Module ConfinedSpace importé avec succès:', !!ConfinedSpaceModule.default);
        
        setConfinedSpaceComponent(ConfinedSpaceModule);
        
      } catch (error) {
        logger.info('⚠️ Erreur chargement ConfinedSpace:', error);
    ...
    logger.info(`Permis sélectionné: ${permitId} - Chemin corrigé`);
  };
  ...
  const handleSavePermit = useCallback((data: any) => {
    logger.info('Sauvegarde du permis:', data);
  ...
  const handleSubmitPermit = useCallback((data: any) => {
    logger.info('Soumission du permis:', data);
  ...
  if (selectedPermit === 'confined-space' && confinedSpaceComponent) {
    ...
    logger.info('Rendu ConfinedSpace avec props:', {
