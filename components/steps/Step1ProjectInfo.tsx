...
function Step1ProjectInfo({ formData, onDataChange, language, tenant, errors = {}, userId, userRole }: Step1ProjectInfoProps) {
  ...
  const notifyParentStable = useCallback((updatedData: any) => {
    ...
    if (lastUpdateRef.current === updateKey) {
      logger.info('🛡️ Step1 - Doublon évité');
      return;
    }
    
    lastUpdateRef.current = updateKey;
    logger.info('🔥 Step1 - Notification parent stable:', Object.keys(updatedData));
  ...
  const updateField = useCallback((field: string, value: any) => {
    logger.info('🔥 Step1 - Update field ULTRA-STABLE:', field, value);
  ...
  const updateLockoutPoint = useCallback((pointId: string, field: string, value: any) => {
    logger.info('🔥 Step1 - Update lockout ULTRA-STABLE:', pointId, field, value);
  ...
  const addWorkLocation = useCallback(() => {
    ...
    logger.info('✅ Step1 - Emplacement ajouté:', location.name);
  ...
  const removeWorkLocation = useCallback((locationId: string) => {
    ...
    logger.info('✅ Step1 - Emplacement supprimé:', locationId);
  ...
  const updateLocationWorkerCount = useCallback((locationId: string, newWorkerCount: number) => {
    ...
    logger.info(`✅ Step1 - Emplacement ${locationId} - Travailleurs: ${newWorkerCount}`);
  }, [localData.workLocations, notifyParentStable]);
  ...
  const processPhoto = useCallback(async (file: File, category: string, lockoutPointId?: string) => {
    try {
      ...
      logger.info('✅ Step1 - Photo ajoutée:', newPhoto.id);
    } catch (error) {
  ...
  const deletePhoto = useCallback((photoId: string) => {
    ...
    logger.info('✅ Step1 - Photo supprimée:', photoId);
  }, [notifyParentStable]);
  ...
  const regenerateASTNumber = useCallback(() => {
    ...
    logger.info('✅ Step1 - Nouveau numéro AST:', newNumber);
  }, []);
  ...
  logger.info('🔥 Step1 - Render avec localData:', Object.keys(localData));
