...
async function callClaude(apiKey: string, system: string, prompt: string, opts?: { tools?: any[]; maxTokens?: number }) {
  const payload: any = { max_tokens: opts?.maxTokens || 2048, system, messages: [{ role: 'user', content: [{ type: 'text', text: prompt }] }] };
  if (opts?.tools) payload.tools = opts.tools;
  const resp = await anthropicMessages(apiKey, payload);
...
export async function POST(req: NextRequest) {
  ...
  const system = [
    ...
    // Produit affilié : la plateforme CERDIA (gestion immobilière / investissement) — autre produit du groupe.
    "PRODUIT CERDIA (plateforme distincte du groupe, à promouvoir quand le sujet le demande) : CERDIA est une PLATEFORME DE GESTION IMMOBILIÈRE ET D'INVESTISSEMENT multi-tenant et assistée par IA. Elle réunit dans un seul tableau de bord : gestion des propriétés d'un parc immobilier international, suivi des investisseurs, valeur nette par part (NAV), trésorerie et conformité fiscale multi-juridiction. Public cible : SOCIÉTÉS DE GESTION immobilière, FAMILY OFFICES et GROUPES D'INVESTISSEURS détenant des actifs au Canada, en République dominicaine, aux États-Unis et au Mexique. Architecture multi-tenant sécurisée (chaque organisation totalement isolée). Fonctions clés : suivi de portefeuille en temps réel (valeur, NAV/part, rendements, appréciation) ; multi-devise automatique (CAD, USD, DOP, MXN, EUR aux taux de la Banque du Canada) ; conformité fiscale intégrée (T1135, T2209, IRNR, ITBIS, FIRPTA, TDT calculés par juridiction) ; gestion des investisseurs (parts, classes d'actions, votes sur scénarios, dividendes, relevés T5) ; trésorerie & rapports (compte courant, flux, échéanciers, rapports comptables exportables) ; assistant IA intégré et confidentiel. Essai : démo gratuite (tableau de bord complet). Tarif : abonnement plateforme multi-tenant 1 450 $/an par organisation. CTA possibles : « Démarrer la démo » ou « Nous contacter ». N'associe PAS CERDIA au persona SST ; cible la finance/immobilier/investissement.",
...
Retourne UNIQUEMENT ce JSON : {"text":"le texte à prononcer","words":<nombre de mots>,"seconds":${seconds}}`;
    ...
    if (action === 'chat') {
      ...
      const resp = await anthropicMessages(apiKey, { max_tokens: 1500, system: chatSystem, messages: msgs });
