...
export function ReferenceStandardsManager({ tenant, tr, canEdit, userEmail }: { tenant: string; tr: Tr; canEdit: boolean; userEmail?: string }) {
  ...
  useEffect(() => {
    getReferenceStandards(tenant).then(rs => {
      ...
      setRows(r); setMeta(rs._meta || {}); setValidated(!!rs._meta?.validated_at); setLoading(false);
    }, () => setLoading(false));
  }, [tenant]);
  ...
  async function save() {
    ...
    const { error } = await saveReferenceStandards(tenant, { ...rs, dga_percentiles, _meta });
