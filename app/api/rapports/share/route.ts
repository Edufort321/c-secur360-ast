...
function isExpired(s: any) { return s.revoked || (s.expires_at && new Date(s.expires_at).getTime() < Date.now()); }
...
export async function POST(req: NextRequest) {
  ...
  const mode = body.mode === 'review' ? 'review' : 'view';
  ...
  const { error } = await supabaseAdmin.from('rapport_shares').insert({ token, report_id: reportId, tenant_id: tenant, mode, created_by: u.email || null, expires_at });
...
export async function GET(req: NextRequest) {
  ...
  if (token) {
    ...
    return NextResponse.json({ ok: true, mode: share.mode, report: { id: rep.id, title: rep.title, status: rep.status, num: rep.num, data: rep.data || {} }, logo, reviews: reviews || [] });
  }
