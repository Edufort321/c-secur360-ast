...
export async function buildJournalRows(tenant: string): Promise<(string | number)[][]> {
  const { data, error } = await supabase.from('gl_entries')
    .select('entry_number, entry_date, description, reference, posted, gl_journals(code), gl_lines(debit, credit, description, gl_accounts(code, name, type))')
    .eq('tenant_id', tenant).eq('posted', true).order('entry_date', { ascending: true });
  if (error) throw error;
  const rows: (string | number)[][] = [[
    'Date', 'N° écriture', 'Journal', 'Référence', 'Description', 'Compte', 'Nom du compte', 'Type', 'Libellé ligne', 'Débit', 'Crédit',
  ]];
  for (const e of (data || []) as any[]) {
    ...
    for (const l of (e.gl_lines || []) as any[]) {
      ...
      rows.push([
        ...
        a.code || '', a.name || '', a.type || '', l.description || '', num(l.debit), num(l.credit),
      ]);
    }
  }
  return rows;
